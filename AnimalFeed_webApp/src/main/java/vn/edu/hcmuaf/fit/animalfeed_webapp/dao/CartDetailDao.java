...
public class CartDetailDao {
    ...
    public List<CartItem> getCartDetailByUser(int id) {
        return jdbi.withHandle(handle -> handle.createQuery(
                        "SELECT cd.*, p.name, p.img, p.price, p.description, p.discount_id FROM cart_details cd JOIN products p ON cd.product_id = p.id WHERE cd.user_id = :id")
                .bind("id", id)
                .map((rs, ctx) -> {
                    ...
                    item.setTotal(rs.getDouble("total")); // Initial total from DB
                    item.setName(rs.getString("name"));
                    item.setImg(rs.getString("img"));
                    item.setPrice(rs.getDouble("price"));
                    item.setDesc(rs.getString("description"));

                    // Recalculate unitPrice and total based on current discount
                    ...
                    item.setUnitPrice((product.getDiscountId() > 1)
                            ? discountDao.calculateDiscountedPrice(item.getPrice(), product.getDiscountId())
                            : item.getPrice());
                    item.setTotal(item.getUnitPrice() * item.getQuantity()); // Override total with recalculated value
